...
export default function DailyTransportDashboard() {
      ...
      <div className="grid grid-cols-3 gap-6 h-[260px]">
        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-white flex flex-col justify-between">
          <div className="flex justify-between items-center mb-2">
            <h3 className="text-base font-black text-slate-800 flex items-center gap-2">
              <BarChart3 className="w-5 h-5 text-indigo-500" />各路线计划与完成车次对比
            </h3>
          ...
          <div className="flex flex-col gap-4 flex-1 justify-center">
            {[
              { name: '杭州-合肥', plan: 120, done: 116 },
              { name: '南京-苏州', plan: 96, done: 88 },
              { name: '合肥-芜湖', plan: 78, done: 74 },
              { name: '上海-杭州', plan: 65, done: 61 },
            ].map((route, idx) => (
              <div key={idx} className="flex items-center gap-4 text-sm font-bold group">
                <span className="w-20 text-slate-600">{route.name}</span>
                <div className="flex-1 h-3 bg-slate-100 rounded-full relative overflow-hidden shadow-inner">
                  <div className="absolute top-0 left-0 bottom-0 bg-indigo-100 rounded-full" style={{ width: `${(route.plan / 120) * 100}%` }} />
                  <div className="absolute top-0 left-0 bottom-0 bg-indigo-500 rounded-full shadow-[0_0_10px_rgba(99,102,241,0.5)] transition-all duration-700 ease-out" style={{ width: isChartMounted ? `${(route.done / 120) * 100}%` : '0%' }} />
                </div>
                <div className="w-16 text-right flex gap-2 justify-end">
                  <span className="text-slate-800 font-black">{route.plan}</span>
                  <span className="text-indigo-600 font-black">{route.done}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-white flex flex-col justify-between relative overflow-hidden">
          <div className="flex justify-between items-center relative z-10">
            <h3 className="text-base font-black text-slate-800 flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-indigo-500" />任务完成率趋势分析
            </h3>
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-white flex flex-col justify-between">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-base font-black text-slate-800 flex items-center gap-2">
              <PieChart className="w-5 h-5 text-emerald-500" />运输车辆状态占比
            </h3>
          </div>
          <div className="flex flex-col gap-5 flex-1 justify-center">
            {[
              { label: '空闲车辆', pct: 48, color: 'bg-indigo-500' },
              { label: '运输中', pct: 36, color: 'bg-blue-500' },
              { label: '异常车辆', pct: 16, color: 'bg-rose-500' },
            ].map((item, idx) => (
      ...
      <div className="flex gap-6">
        <div className="w-[380px] flex flex-col gap-6 shrink-0">
          <div className="bg-gradient-to-br from-indigo-600 to-blue-700 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-xl shadow-indigo-200">
            ...
            <div className="relative z-10">
              <h2 className="text-[15px] font-black opacity-90 mb-8 flex items-center gap-2">
                <Star className="w-5 h-5 fill-white" /> 全项目概览
              </h2>
              <div className="grid grid-cols-2 gap-y-8 gap-x-4">
                <div className="flex flex-col gap-1"><span className="text-[11px] font-bold uppercase tracking-widest opacity-70">计划车次</span><span className="text-4xl font-black tracking-tight">1066</span></div>
                <div className="flex flex-col gap-1"><span className="text-[11px] font-bold uppercase tracking-widest opacity-70">完成车次</span><span className="text-4xl font-black tracking-tight text-emerald-300 drop-shadow-sm">70</span></div>
                <div className="flex flex-col gap-1"><span className="text-[11px] font-bold uppercase tracking-widest opacity-70">总营收 (元)</span><span className="text-3xl font-black tracking-tight">765,000</span></div>
          ...
          <div className="bg-white rounded-[2.5rem] p-4 shadow-sm border border-white flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-2">
            {[
              { name: '研发中心', plan: 0, done: 0, active: true },
              { name: '南京车队', plan: 252, done: 26, active: false },
              { name: '合肥车队', plan: 438, done: 6, active: false },
              { name: '项目一公司', plan: 376, done: 38, active: false },
            ].map((proj, idx) => (
              <div
                key={idx}
              ...
              >
                <div className="flex flex-col gap-2">
                  ...
                  <div className={`text-[11px] font-bold ${proj.active ? 'text-indigo-600' : 'text-slate-400'} flex gap-4`}>
                    <span>计划 <span className="text-sm font-black ml-1">{proj.plan}</span></span>
                    <span>完成 <span className="text-sm font-black ml-1">{proj.done}</span></span>
                  </div>
                </div>
                {proj.active && (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div className="bg-slate-50/50 rounded-3xl p-6 border border-slate-100">
                  ...
                  <div className="space-y-5 text-sm font-bold text-slate-600">
                    ...
                    <div className="flex justify-between items-end"><span>计划车次</span><p className="text-2xl font-black text-slate-800">0 <span className="text-xs">次</span></p></div>
                    <div className="flex justify-between items-end"><span>完成车次</span><p className="text-2xl font-black text-indigo-600">0 <span className="text-xs">次</span></p></div>
                  </div>
                </div>
                <div className="bg-slate-50/50 rounded-3xl p-6 border border-slate-100">
                ...
                <div className="bg-slate-50/50 rounded-3xl p-6 border border-slate-100 relative overflow-hidden">
                  ...
                  <span className="text-[12px] font-black text-slate-400 uppercase tracking-widest mb-6 block relative z-10">运输价值</span>
                  <div className="space-y-5 relative z-10 text-sm font-bold text-slate-600">
                    ...
                    <div className="flex justify-between items-end pt-2 mt-2 border-t border-slate-200"><span className="text-sm font-black text-slate-800">单位收益</span><p className="text-3xl font-black text-amber-500 drop-shadow-sm">0</p></div>
                  </div>
                </div>
              </div>
            </div>
